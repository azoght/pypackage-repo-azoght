...
def register_or_update(pkg_name, version, author, short_desc, homepage):
    """Register a new package or update an existing one"""
    # Read our index first to check if package exists
    with open(INDEX_FILE) as html_file:
        soup = BeautifulSoup(html_file, "html.parser")
    norm_pkg_name = normalize(pkg_name)
    
    if package_exists(soup, norm_pkg_name):
        print(f"Package {norm_pkg_name} already exists. Updating to version {version}...")
        update(pkg_name, version)
    else:
        print(f"Package {norm_pkg_name} doesn't exist. Registering new package...")
        register(pkg_name, version, author, short_desc, homepage)
...
def main():
    ...
    elif action == "REGISTER_OR_UPDATE":
        register_or_update(
            pkg_name=os.environ["PKG_NAME"],
            version=os.environ["PKG_VERSION"],
            author=os.environ["PKG_AUTHOR"],
